...
const BuddyUpEventDescription = () => {
  ...
  const [buddyUpDetails, setBuddyUpDetails] =
    useState<GroupActivityDetail | null>(null);
  ...
  const { buttonText, isDisabled, shouldShowApprovalButtons } = useMemo(() => {
    if (!buddyUpDetails) {
      return {
        buttonText: t("join"),
        isDisabled: true,
        shouldShowApprovalButtons: false,
      };
    }
    ...
    if (isEnded && isCreator) {
      ...
      if (!hasParticipants) {
        return {
          buttonText: t("pendingreschedule"),
          isDisabled: false,
          shouldShowApprovalButtons: false,
        };
      }
  ...
  const handleButtonAction = useCallback(async () => {
    ...
    if (isEnded && isCreator) {
      ...
      const params: Record<string, string> = {
        ...
        hashtags: buddyUpDetails.hashtags
          ? JSON.stringify(buddyUpDetails.hashtags)
          : "",
    ...
    try {
      ...
      const response = await addeditdeletebuddyupevent({
        groupActivities: [
          { eventId: buddyUpDetails.id, joinedPeople: updatedJoined },
        ],
      });
    ...
    <View style={styles.main}>
      <GlobalHeader
        ...
        onLeftPress={() =>
          router.canGoBack()
            ? router.back()
            : router.replace("/(bottomtab)/shiplife")
        }
      />
      ...
      {!isOnline ? (
        ...
        >
          <View style={styles.imageView}>
            <Image
              source={
                buddyUpDetails.imageUrls?.[0] || ImagesAssets.PlaceholderImage
              }
          ...
          <View style={styles.section}>
            ...
            <View style={styles.rowBetween}>
              <Text style={styles.organizerText}>
                {t("organizedBy")}{" "}
                {isCreator ? t("you") : buddyUpDetails.activityUser.fullName}
              </Text>
