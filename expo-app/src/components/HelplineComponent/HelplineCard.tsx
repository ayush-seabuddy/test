...
const APP_KEY = process.env.EXPO_PUBLIC_FRESHCHAT_APP_KEY!;
const DOMAIN = process.env.EXPO_PUBLIC_FRESHCHAT_DOMAIN!;
    ...
    const setupFreshchat = async () => {
      try {
        ...
        if (userData) {
          ...
          Logger.info(
            `Freshchat ready → ${fcUser.firstName} ${fcUser.lastName}`,
          );
        } else {
          Freshchat.setUser(fcUser);
