...
const TopThreeEmployees = ({ topEmployee }: { topEmployee: TopEmployee[] }) => {
  ...
  if (!topEmployee || topEmployee.length === 0) {
    return null;
  }
        ...
        renderItem={({ item }) => (
          ...
          >
            <Image
              ...
              style={styles.topEmployeeImage}
              placeholder={ImagesAssets.userIcon}
            />
            <Text style={styles.topEmployeeName}>
              {item.rewardPoints} {t('miles')}
            </Text>
            <Text style={styles.topEmployeeName}>
