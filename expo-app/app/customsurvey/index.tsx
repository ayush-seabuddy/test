...
const SurveyScreen = () => {
  ...
  const [datePickerMode, setDatePickerMode] = useState<
    "date" | "time" | "datetime"
  >("date");
  const [datePickerQuestion, setDatePickerQuestion] = useState<string | null>(
    null,
  );
  ...
  const fetchSurveyQuestions = async () => {
    ...
    try {
      ...
      if (
        ...
        showToast.error(
          t("oops"),
          apiResponse.message || t("failedToLoadSurvey"),
        );
      }
    } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    if (missing.length > 0) {
      showToast.error(t("oops"), t("pleaseanswerall"));
      return;
    }
    ...
    try {
      const responseJson = Object.entries(answers).map(
        ([questionId, answer]) => ({
          questionId,
          answer,
        }),
          ...
          {q.answerType === "yesno" && (
            <View style={styles.yesNoContainer}>
              ...
              >
                <Text
                  style={
                    val === true ? styles.optionTextSelected : styles.optionText
                  }
                >
                  Yes
                </Text>
              ...
              <TouchableOpacity
                style={[
                  styles.yesNoBtn,
                  val === false && styles.optionSelected,
                ]}
          ...
          {q.answerType === "range1-5" && (
            <View>
              ...
              <View style={styles.rangeLabelsContainer}>
                {[1, 2, 3, 4, 5].map((num) => (
                  <Text key={num} style={styles.rangeLabel}>
                    {num}
                  </Text>
                ))}
              </View>
