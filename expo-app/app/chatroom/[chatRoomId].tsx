...
const ChatRoomScreen = () => {
  ...
  const chatRoomDetails =
    typeof route.params?.chatRoomDetails === "string"
      ? JSON.parse(route.params?.chatRoomDetails)
      : route.params?.chatRoomDetails;
  const chatRoomId = chatRoomDetails?.id || route.params?.chatRoomId;
  const [participant, setParticipant] = useState(
    chatRoomDetails?.participantIds,
  );
  ...
  const { id: senderId, shipId } = useSelector(
    ...
    setChatList((prevMessageList) => {
      return prevMessageList.map((msg) => {
        if (msg.id === message.id) {
          ...
          let alreadyReacted = msg?.chatReactionDetails?.find(
            (item) => item.userId === senderId,
          );
          if (alreadyReacted) {
            if (alreadyReacted.reaction == emoji) {
              chatReactionDetails = chatReactionDetails?.filter(
                (item) => item.userId !== senderId,
              );
            } else {
              chatReactionDetails = chatReactionDetails?.filter(
  ...
  const handleDeleteReaction = (messageId: string, emoji: string) => {
    const message = chatListState.find(
  ...
  const fetchChatReactions = async (id: string) => {
    try {
      ...
      if (response.data && response.data.length > 0) {
        let myReaction: ReactionData = response.data.find(
          (reaction: ReactionData) => String(reaction.userId) == senderId,
        );
        let reactionWithoutMy = response.data.filter(
          (reaction: ReactionData) => String(reaction.userId) != senderId,
        );
        if (myReaction) {
  ...
  const handleGetUserMessage = (message: ChatMessage) => {
    setChatList((prevMessageList) =>
      prevMessageList.map((msg) =>
        String(msg.createdAtId) === String(message.createdAtId)
  ...
  const sendMessageImageUrl = async (
    contentImage: string,
    chat_payload: {
      senderId: string;
      chatRoomId: string;
  ...
  const uploadImage = async (
    ...
    try {
      ...
      const apiResponse = await uploadfile({
        file: photo,
        fileName: fileName,
        fileSize: fileSize,
        type: type,
                    ...
                    <View style={styles.userItemContainer}>
                      <View style={{ position: "relative" }}>
                        <Image
                          ...
                          source={
                            user?.reactUsers?.profileUrl
                              ? { uri: user?.reactUsers?.profileUrl }
                              : null
                          }
                          contentFit="cover"
                          onTouchEnd={() => {
                ...
                <FlatList
                  ...
                  renderItem={({ item: emoji }) => (
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        editingMessage &&
                        handleEmojiReaction(editingMessage, emoji)
                      }
                    >
