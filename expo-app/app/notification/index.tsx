...
const NotificationScreen = () => {
    ...
    const fetchNotifications = useCallback(
        async (pageNum: number, isLoadMore = false) => {
            ...
            try {
                const response = await getallnotifications({ page: pageNum, limit: 10 });
    ...
    const deleteSingleNotification = useCallback(async () => {
        ...
        try {
            const response = await deleteandclearallnotification({ notificationId: selectedNotificationId });
    ...
    const clearAllNotifications = useCallback(async () => {
        ...
        try {
            ...
            if (response.success && response.status === 200) {
                setNotifications([]);
                showToast.success(t('success'), t('allnotificationdeletedsuccessfully'));
            } else {
                showToast.error(t('oops'), response.message);
            }
        } catch (error) {
    ...
    return (
        <View style={styles.main}>
            ...
            <Modal visible={deleteSingleModalVisible} transparent animationType="fade">
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
