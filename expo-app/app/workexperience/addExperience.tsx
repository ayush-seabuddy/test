...
const AddExperienceScreen = () => {
  ...
  const formatDateForDisplay = (date: Date | null): string => {
    ...
    try {
      ...
      Logger.error("Error formatting date:", { Error: String(error) });
      return "";
    }
  };
  ...
  const formatDateForAPI = (date: Date | null): string => {
    ...
    try {
      ...
      Logger.error("Error formatting date for API:", {Error:String(error)});
      return "";
    }
  };
    ...
    try {
      ...
      if (isEdit && experienceId) {
        ...
        updatedExperiences = currentExperiences.map((exp: any) =>
          exp.id === experienceId
            ? ({ ...exp, ...newExperience } as WorkExperience)
            : exp,
        );
      } else {
        // Add new experience
      ...
      if (response.status === 200) {
        ...
        if (result?.data) {
          ...
          for (const property in object) {
            dispatch(
              updateUserField({ key: property, value: object[property] }),
            );
          }
        }
        showToast.success(
          t("success"),
          isEdit
            ? t("workingexperienceupdatedsuccessfully")
            : t("workingexperienceaddedsuccessfully"),
        );
        router.back();
      }
    } catch (error: any) {
      Logger.error("Save error:", error.response?.data || error.message);
