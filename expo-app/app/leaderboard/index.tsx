...
const LeaderboardScreen = () => {
  ...
  const [sailorsOfTheMonth, setSailorsOfTheMonth] = useState<
    SailorsOfTheMonth[]
  >([]);
  const [overallCrewListing, setOverallCrewListing] = useState<
    OverallCrewRanking[]
  >([]);
  ...
  const isSailorsVisible = useMemo(
    () => sailorsOfTheMonth.length > 0,
    [sailorsOfTheMonth],
  );
  ...
  const designationDropDown = useMemo(
    () => [
      ...
      {
        label: t("designationDropDown.second_officer"),
      ...
      {
        label: t("designationDropDown.ordinary_seaman"),
      ...
      {
        label: t("designationDropDown.chief_engineer"),
        value: "Chief engineer",
      },
      {
        label: t("designationDropDown.second_engineer"),
        value: "Second engineer",
      },
      {
        label: t("designationDropDown.third_engineer"),
        value: "Third engineer",
      },
      {
        label: t("designationDropDown.fourth_engineer"),
        value: "Fourth engineer",
      },
      {
        label: t("designationDropDown.junior_engineer"),
        value: "Junior engineer",
      },
      { label: t("designationDropDown.engine_cadet"), value: "Engine cadet" },
      {
        label: t("designationDropDown.electrical_engineer"),
        value: "Electrical engineer / ETO",
      },
      {
        label: t("designationDropDown.electrical_cadet"),
        value: "Electrical cadet",
      },
      { label: t("designationDropDown.engine_fitter"), value: "Engine fitter" },
      {
        label: t("designationDropDown.motorman_oiler"),
        value: "Motorman/ Oiler",
      },
  ...
  const getAllShipsList = async () => {
    ...
    setLoading(true);
    try {
      const apiResponse = await getallshipslist({
        ...
        if (isLoadMore) {
          ...
          const newUsers = usersList.map((user: any, idx: number) => {
            ...
            // Check if same points as last user in existing list
              ...
              previousLength > 0 &&
              points ===
                Number(overallCrewListing[previousLength - 1].rewardPoints)
            ) {
          ...
          const sorted = [...usersList].sort(
            (a: any, b: any) => Number(b.rewardPoints) - Number(a.rewardPoints),
          );
          ...
          const ranked = sorted.map((user: any, idx: number) => {
            if (
              idx > 0 &&
              Number(user.rewardPoints) !== Number(sorted[idx - 1].rewardPoints)
            ) {
              rank = idx + 1;
            }
            return { ...user, rank };
  ...
  const renderSailorItem = useCallback(
    ({ item, index }: { item: SailorsOfTheMonth; index: number }) => (
      <ActiveSailorsCard
  ...
  const keyExtractorSailor = useCallback(
    (_: any, i: number) => `sailor-${i}`,
    [],
  );

  return (
    <View style={styles.main}>
      <GlobalHeader title={t("leaderboard")} />
                ...
                <View style={styles.sailorsHeaderContainer}>
                  <View style={styles.textContainer}>
                    <View style={styles.titleRow}>
                      ...
                      <GlobalPopOver
                        popOverText={t("leaderboardinfo_description")}
                      >
                        <InfoIcon
                          size={16}
                          color="#555"
