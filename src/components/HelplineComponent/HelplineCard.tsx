...
const APP_ID = env.freshchatAppId;
const APP_KEY = env.freshchatAppKey;
const DOMAIN = env.freshchatDomain;
...
const HelplineAndAICards = () => {
	...
	useEffect(() => {
		const setupFreshchat = async () => {
			try {
				if (!APP_ID || !APP_KEY || !DOMAIN) {
					Logger.warn("Freshchat env vars are missing. Skipping Freshchat init.");
					return;
				}
				...
				if (userData) {
					...
					fcUser.phoneCountryCode =
						userData.countryCode || userData.phoneCountryCode || "+91";
			...
			Freshchat.getUnreadCountAsync((data) => {
				if (data?.status)
					Logger.info("Unread messages:", {
						UnreadMessageCount: String(data.count ?? 0),
					});
			});
		};
				...
				<View style={styles.card}>
					<View style={styles.content}>
						<Image
							...
							style={styles.icon}
							contentFit="contain"
							placeholder={ImagesAssets.helplineFormIcon}
