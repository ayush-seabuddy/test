...
const BuddyUpEventCard = ({
  ...
  const [loggeduserData, setloggeduserData] = useState<{ id: string } | null>(
    null,
  );
  ...
  const handleEditClick = (event: BuddyUpEvent) => {
    ...
    router.push({
      ...
      params: {
        ...
        invitedPeople: event.invitedPeoples
          ? JSON.stringify(event.invitedPeoples)
          : "[]",
        joinedPeople: event.joinedPeople
          ? JSON.stringify(event.joinedPeople)
          : "[]",
        invitedPeoples: event.invitedPeoples
          ? JSON.stringify(event.invitedPeoples ?? [])
          : "[]",
      },
    });
    ...
    <View style={styles.main}>
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          const couldEditDelete =
            isCreatedByMe && moment().isBefore(item.startDateTime);
          ...
          return (
            ...
            >
              <Image
                source={item.imageUrls[0] || ImagesAssets.SeabuddyPlaceholder}
              ...
              <Text style={styles.buddyupeventName} numberOfLines={1}>
                {item.eventName}
              </Text>
              <Text style={styles.organizedby} numberOfLines={1}>
                {t("organizedBy")}{" "}
                {isCreatedByMe ? t("you") : item.activityUser.fullName}
              </Text>
