...
class WSService {
	...
	initializeSocket = async (): Promise<void> => {
		try {
			...
			if (this.socket?.connected || this.socket?.id) {
				...
				Logger.info("Socket already connected or initialized", {
					SocketId: String(this.socket.id),
				});
				return;
			}
	...
	on = (event: SocketEvent, callback: SocketCallback): void => {
		if (this.socket) {
			...
			console.error("Socket not initialized. Cannot register listener for:", event);
			Logger.error("Socket not initialized. Cannot register listener for", {
				event,
			});
		}
	};
